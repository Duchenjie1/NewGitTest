...
class Bottleneck(nn.Module):  #就是Block
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=32, base_width=4, reduction=16):  #reduction
        ...
        # c2wh = dict([(64, 56), (128, 28), (256, 14), (512, 7)])  #resnet,由于输入图像随机裁剪为224*224,第64层的特征图长*宽：56*56; 第128层的特征图长*宽：28*28；以此类推
        c2wh = dict([(64, 56), (128, 28), (256, 14), (512, 7)])  #由于输入图像随机裁剪为224*224,第64层的特征图长*宽：56*56; 第128层的特征图长*宽：28*28；以此类推
    ...
    def forward(self, x):
        ...
        out = self.bn1(out)
        out = self.relu(out)
        # F1 = self.feature_extraction(out)  #GAP
        F1 = self.feature_extraction1(out)  #Fcanet
