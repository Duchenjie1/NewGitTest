...
class RandomSampleCrop(object):
    ...
    def __call__(self, sample):
        ...
        target_size = 512
        # target_size = 224  #du revised:input[224,224]
        if shorter_side < 384: 
            target_size = 256
        min_iou = 0.2
        crop_success = False
...
class AspectRatioBasedSampler(Sampler):
    ...
    def __init__(self, data_source, batch_size, drop_last):
        ...
        self.groups = self.group_images()   #调用group_images函数
    ...
    def group_images(self):
        ...
        order.sort(key=lambda x: self.data_source.image_aspect_ratio(x))   #调用image_aspect_ratio函数，按照图像分辨率的宽高比进行排序(1.333(MOT20-03), 1.531(MOT20-05),1.777(MOT20-01和02))
