...
def track_det_match(tracklet_list, det_rect_list, min_iou = 0.5):  #
	...
	match_result = np.transpose(match_result)  #跟踪-检测对 [[0，0],[1，1],[2，2],[3，3],[4，7],[5，5],[6，6],[7，11],[8，9],[9，4],[10，10],[11，8]，[12，13],[13，14],[14，15],[15，12],[17，18],[18，17],[19，19],[20，16],[21,21],[22，20],[23，22]]
...
def run_each_dataset(model_dir, retinanet, dataset_path, subset, cur_dataset):	
	...
	for idx in range(img_len + 1):
		...
		with torch.no_grad():
			...
			resize_h, resize_w = math.ceil(img_h / 32) * 32, math.ceil(img_w / 32) * 32  #使图像的长、宽是32的倍数. math.ceil:向上取整，即小数部分直接舍去，并向整数部分进1
			img1 = np.zeros((resize_h, resize_w, 3), dtype=img_origin1.dtype)  #[1088，1920，3]
			img1[:img_h, :img_w, :] = img_origin1
			img1 = (img1.astype(np.float32) / 255.0 - np.array([[RGB_MEAN]])) / np.array([[RGB_STD]]) #astype:对数据类型进行转换，除以255就是归一化
			...
			if idx > 0:
				...
				matches, unmatched1, unmatched2 = track_det_match(tracklet_all, det_list_all[i], IOU_threshold)  #跟踪-检测匹配(IOU匹配). matches:匹配的跟踪-检测对，unmatched1：未匹配的跟踪，unmatched2：未匹配的检测

				for j in range(len(matches)):
					det_list_all[i][matches[j][1]].id = tracklet_all[matches[j][0]].id  #将匹配的跟踪轨迹id赋值给检测结果，det_list_all[i][n]:表示第i帧第n个成对pred_bbox. matches[j][1]:第j个跟踪-检测对中的检测
					det_list_all[i][matches[j][1]].id = tracklet_all[matches[j][0]].id
					tracklet_all[matches[j][0]].add_rect(det_list_all[i][matches[j][1]])  #跟踪轨迹池中添加完成新匹配的检测结果. matches[j][0]:第j个跟踪-检测对中的跟踪. 调用add_rect()函数

				delete_track_list = []  #删除的轨迹
				for j in range(len(unmatched1)): #unmatched1：未匹配的跟踪，例如第16号
					tracklet_all[unmatched1[j]].no_match_frame = tracklet_all[unmatched1[j]].no_match_frame + 1  #no_match_frame：未匹配帧的数量
	...
	videoWriter = cv2.VideoWriter(out_video, cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), fps, (img_width, img_height))  #cv2.VideoWriter:写视频. cv2.VideoWriter_fourcc('m', 'p', '4', 'v')：相对较旧的MPEG-4编码。如果要限制结果视频的大小，这是一个很好的选择。文件扩展名应为.m4v
	...
	for i in range(img_len):  #第i+1帧
		...
		for j in range(len(det_list_all[i])):  #检测结果列表中第i+1帧第j+1个成对pred_bbox
			...
			id_dict.setdefault(str(trace_id),[]).append((int((x1+x2)/2), y2))  #字典设置默认值，例如：{'1':[(718,899),(718,902),(),...],'2':[799,891][],'3':[][]}，主要用于画线
			draw_trace_id = str(trace_id)
			draw_caption(img, (x1, y1, x2, y2), draw_trace_id, color=color_list[trace_id % len(color_list)])  #调用draw_caption()函数
			...
			for k in range(trace_len_draw - draw_interval):  #draw_interval：5
				if(k % draw_interval == 0):  #画轨迹(每间隔5帧画一直线)
					draw_point1 = id_dict[str(trace_id)][trace_len - k - 1]  #轨迹线的第1个点的位置: 第一个目标(id=1)在第6帧时的位置(715, 907)
					draw_point2 = id_dict[str(trace_id)][trace_len - k - 1 - draw_interval]  #轨迹线的第二个点的位置: 第一个目标(id=1)在第1帧时的位置(718, 899)
					cv2.line(img, draw_point1, draw_point2, color=color_list[trace_id % len(color_list)], thickness=2)  #画线，将两个时刻的点连接起来，形成轨迹
...
def run_from_train(model_dir, root_path):
	...
	'''
    #MOT20
	for seq_num in [1, 2, 3, 5]:
		run_each_dataset(model_dir, retinanet, root_path, 'train', 'MOT20-{:02d}'.format(seq_num))
	for seq_num in [4, 6, 7, 8]:
		run_each_dataset(model_dir, retinanet, root_path, 'test', 'MOT20-{:02d}'.format(seq_num))
...
def main(args=None):
	...
	parser.add_argument('--dataset_path', default='/home/du/PycharmProjects/CTracker-master/MOT20_ROOT/', type=str,
	...
	# myinput = torch.randn(1, 3, 224, 224).to('cuda')
	# flops, params = profile(retinanet.to('cuda'), inputs=(myinput,))
    ...
    #MOT20
	for seq_num in [1, 2, 3, 5]:
		run_each_dataset(parser.model_dir, retinanet, parser.dataset_path, 'train', 'MOT20-{:02d}'.format(seq_num))
	for seq_num in [4, 6, 7, 8]:
		run_each_dataset(parser.model_dir, retinanet, parser.dataset_path, 'test', 'MOT20-{:02d}'.format(seq_num))
